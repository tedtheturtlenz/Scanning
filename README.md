...
There are 2 main batch files, Capture_and_Process_and_Segment, which is used for Bonsai Trees, and Capture_and_Process_No_Plant, which is used for general scanning. CaPaS will do a full scan, reconstruct the Bonsai, and segment the 3D model. CaP-NP will omit the last step, so it can be used for scanning objects other than Bonsai Trees.
